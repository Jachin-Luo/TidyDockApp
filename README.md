...
Phase 1 is complete. The app is usable as a WPF/.NET Framework desktop application with a portable package and a single-file current-user installer.

## Features

- Manual Dock items: apps, shortcuts, folders, files, URLs, and separators
- No desktop scanning, no app monitoring, no recent files, no background indexing
...
- Folder stack popup with lazy directory reads and quick auto-hide
- Local JSON config and local icon cache
- Tray menu, startup option, and current-user installer
...
```powershell
powershell -ExecutionPolicy Bypass -File ".\TidyDockApp\scripts\Test-Portable.ps1"
```
