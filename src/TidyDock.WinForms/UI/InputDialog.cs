...
internal sealed class InputDialog : Form
{
    ...
    public InputDialog(string title, string label, string value, ThemePalette palette)
    {
        ...
        buttons.Controls.Add(MakeButton("OK", palette, true, delegate { DialogResult = DialogResult.OK; }));
        buttons.Controls.Add(MakeButton("Cancel", palette, false, delegate { DialogResult = DialogResult.Cancel; }));
    ...
    private static Button MakeButton(string text, ThemePalette palette, bool primary, EventHandler click)
    {
        var button = new Button
        {
            ...
            BackColor = primary ? palette.Accent : palette.PanelAlt,
